...
export const GameProps = {
  ...
  dayTime: true,
  speed: 1,
  status: EGameStatus.idle, //for testing
  background: null,
  ground: null,
  hero: null,
  obstacles: [],
  ...
  score: 0,
  bestScore: 0,
  // sounds: {contDown: null, food: null, gameOver: null, dead: null, running: null},
};
...
/*denne seksjonen er i effekt mine "Layers". vis eg trenger å endre på layers bare flytt opp og ned*/ 
function drawGame() {
  spcvs.clearCanvas();
...
for (let i = 0; i < GameProps.obstacles.length; i++) {
  ...
  if(obstacle.right < GameProps.hero.left && !obstacle.hasPassed) {
    //Gratulere, du har kommet forbi hindringen!!
    GameProps.menu.incScore(20);
    console.log("Score: " + GameProps.menu.score);
